...
import { Plus, ArrowDownLeft, Trash2, Edit3, Tag } from 'lucide-react';
...
export const Income = ({ selectedMonth, onRefresh }) => {
  ...
  const loadData = async () => {
    ...
    try {
      const [txRes, catRes] = await Promise.all([
        transactionAPI.getAll({ month: selectedMonth, type: 'income' }),
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Track salary, freelance, business revenue, and incoming cashflows</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatCard
          ...
          icon={Tag}
          color="purple"
          subtitle="Salary, Freelance, Business"
        />
      </div>

      {/* Income Table */}
      <Card title={`Income Records (${selectedMonth})`}>
        {loading ? (
          <LoadingSpinner label="Fetching income entries..." />
        ) : incomes.length === 0 ? (
          <EmptyState
            title="No Income Recorded"
            description="No income transactions logged for this month yet."
      ...
      {/* Category Modal */}
      <Modal isOpen={isCategoryModalOpen} onClose={() => setIsCategoryModalOpen(false)} title="Add Custom Income Category">
        <form onSubmit={handleCreateCategory} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Dividends, Crypto"
